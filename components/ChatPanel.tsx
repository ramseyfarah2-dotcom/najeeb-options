...
function BullFace({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 64 64" className={className} fill="none">
      ...
      <circle cx="24" cy="28" r="3" fill="#1a1a2e"/><circle cx="40" cy="28" r="3" fill="#1a1a2e"/>
      <circle cx="25" cy="27" r="1" fill="white"/><circle cx="41" cy="27" r="1" fill="white"/>
...
export default function ChatPanel() {
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      setMessages(prev => prev.map(m => m.id === assistantMsg.id ? { ...m, content: `Error: ${err instanceof Error ? err.message : 'Something went wrong'}` } : m))
    } finally {
      setLoading(false)
    }
    ...
    <>
      {!isOpen && (
        <div className="fixed bottom-20 sm:bottom-6 right-4 sm:right-6 z-50 flex flex-col items-end gap-2">
          ...
          <button onClick={() => setIsOpen(true)} className="w-16 h-16 rounded-full bg-gradient-to-br from-[var(--accent)] to-emerald-700 flex items-center justify-center shadow-xl hover:shadow-2xl hover:scale-105 transition-all active:scale-95 relative">
            <BullFace className="w-10 h-10" />
        ...
        <div className="fixed inset-0 sm:inset-auto sm:bottom-6 sm:right-6 z-50 sm:w-[420px] sm:h-[560px] flex flex-col bg-[var(--bg-surface)] sm:rounded-2xl sm:border sm:border-[var(--border)] sm:shadow-2xl animate-slideUp">
          <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border)]">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[var(--accent)] to-emerald-700 flex items-center justify-center">
                <BullFace className="w-5.5 h-5.5" />
              </div>
              <div>
                <p className="text-sm font-semibold text-[var(--text-primary)]">Wall St. Bull</p>
                <p className="text-[10px] text-[var(--text-muted)]">{loading ? '🤔 Thinking...' : 'Your trading assistant'}</p>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3 flex flex-col gap-3">
            {messages.length === 0 && (
              <div className="flex-1 flex flex-col items-center justify-center text-center gap-4 py-6">
                <div className="w-20 h-20 rounded-full bg-gradient-to-br from-[var(--accent)] to-emerald-700 flex items-center justify-center">
                  <BullFace className="w-12 h-12" />
                </div>
                <div>
                  <p className="text-sm font-semibold text-[var(--text-primary)]">Hey Najeeb!</p>
                  <p className="text-xs text-[var(--text-muted)] mt-1">Ask me anything about your portfolio</p>
                </div>
            ...
            {messages.map(msg => (
              <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] px-3 py-2 rounded-xl text-sm whitespace-pre-wrap ${
                  msg.role === 'user' ? 'bg-[var(--accent)] text-[var(--bg-base)]' : 'bg-[var(--bg-elevated)] text-[var(--text-primary)]'
                }`}>
                  {msg.content || (loading && msg.role === 'assistant' ? <Loader2 className="w-4 h-4 animate-spin text-[var(--text-muted)]" /> : null)}
                </div>
              </div>
            ))}
          ...
          <div className="px-4 py-3 border-t border-[var(--border)]">
            <div className="flex gap-2">
              <input ref={inputRef}
                ...
                placeholder="Ask about your positions..." disabled={loading}
              />
